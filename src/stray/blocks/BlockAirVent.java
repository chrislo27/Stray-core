...
public class BlockAirVent extends BlockDirectional {
	...
	public void tickUpdate(World world, int x, int y) {
		...
		if (world.getMeta(x, y) <= SolidFaces.UP) {
		...
		} else if ((world.getMeta(x, y) & SolidFaces.DOWN) == SolidFaces.DOWN) {
			for (int i = 0; i < range; i++) {
				if ((world.getBlock(x, y + i + 1).isSolid(world, x, y + i + 1) & SolidFaces.UP) == SolidFaces.UP) {
					newRange = i;
					break;
				}
			}
		...
		} else if ((world.getMeta(x, y) & SolidFaces.LEFT) == SolidFaces.LEFT) {
			for (int i = 0; i < range; i++) {
				if ((world.getBlock(x - i - 1, y).isSolid(world, x - i - 1, y) & SolidFaces.RIGHT) == SolidFaces.RIGHT) {
					newRange = i;
					break;
				}
			}
		...
		} else if ((world.getMeta(x, y) & SolidFaces.RIGHT) == SolidFaces.RIGHT) {
			for (int i = 0; i < range; i++) {
				if ((world.getBlock(x + i + 1, y).isSolid(world, x + i + 1, y) & SolidFaces.LEFT) == SolidFaces.LEFT) {
					newRange = i;
					break;
				}
			}
