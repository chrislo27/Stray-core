	...
	public void render(float delta) {
		...
		if (Gdx.input.isKeyJustPressed(Keys.SPACE)) {
			if (tiles[selx][sely].equals(choose)) {
				amount--;
				score += 5;
				main.manager.get(AssetMap.get("colour200pts"), Sound.class).play(0.1f);
				if (amount <= 0) {
				...
				main.manager.get(AssetMap.get("colourincorrect"), Sound.class).play(0.5f);
			}
		} else if (Gdx.input.isKeyJustPressed(Keys.R)) {
			if (nuclearness == 0) {
				nuclearness = 52;
				main.manager.get(AssetMap.get("colourswap"), Sound.class).play(0.333f);
			}
		}
